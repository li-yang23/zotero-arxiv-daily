...
- Papers are reviewed by LLM for research quality before delivery, using top AI conference criteria.
...
zotero:
  ...
  api_max_retries: 5 # Number of retries for transient Zotero API 429/5xx failures. Example: 5
  api_retry_delay_seconds: 30 # Delay between Zotero API retries in seconds. Example: 30
...
quality_filter:
  enabled: true # Whether to use LLM review to filter papers by research quality. Example: true
  min_score: 7.0 # Minimum overall quality score required for email delivery. Example: 7.0
  candidate_multiplier: 3 # Review this many times max_paper_num after relevance reranking. Example: 3
...
## 📖 How it works
*Zotero-arXiv-Daily* firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's abstract via an embedding model. The relevance score of a paper is its weighted average similarity over all your Zotero papers (newer paper added to the library has higher weight). After relevance reranking, the LLM reviews candidate papers as a senior top-AI-conference reviewer, extracts the problem, method, and conclusion, scores innovation, rigor, significance, and overall quality, and filters out papers below `quality_filter.min_score`. The TLDR of each delivered paper is generated by LLM, given the text extracted by pymupdf4llm.
